			...
			if(fermion_list[i][j] != null) 
			{
				if(
					...
					fermion_list[i][j].y < bullet_sprite.y + bullet_sprite.height )
					{
						...
						// checking for UD-
						if(current_particle_type_str.includes("up") && (current.includes("down") && current.includes("anti")) ||
								current.includes("up") && (current_particle_type_str.includes("down") && current_particle_type_str.includes("anti"))) {
							update_pion();
						}

						// checking for DU- 
						if(current_particle_type_str.includes("down") && (current.includes("up") && current.includes("anti")) ||
							current.includes("down") && (current_particle_type_str.includes("up") && current_particle_type_str.includes("anti"))) {
							update_antipion();
						}
					}
